...
const debouncedLoadUsage = (state: UsageState) => {
  ...
  usageDateDebounceTimeout = window.setTimeout(() => void loadUsage(state), 140);
};
...
export function renderUsageTab(state: AppViewState) {
  ...
  const analyticsView = buildUsageAnalyticsViewModel({
  ...
  return renderUsage({
    ...
    usageSectionCollapsed: state.usageSectionCollapsed,
    onStartDateChange: (date) => {
      state.usageStartDate = date;
      state.usageSelectedDays = [];
      state.usageSelectedHours = [];
      state.usageSelectedSessions = [];
      state.usageRangePreset = "custom";
    ...
    onRangePresetChange: (preset) => {
      state.usageRangePreset = preset;
      if (preset !== "custom") {
        const range =
          preset === "today"
            ? localCurrentDayUtcCoverageRange()
            : lastNUtcDaysRange(preset === "7d" ? 7 : 30);
        state.usageStartDate = range.startDate;
        state.usageEndDate = range.endDate;
      }
      state.usageSelectedDays = [];
      state.usageSelectedHours = [];
      state.usageSelectedSessions = [];
    ...
    onToggleUsageSection: (section) => {
      state.usageSectionCollapsed = {
        ...state.usageSectionCollapsed,
        [section]: !state.usageSectionCollapsed[section],
      };
    },
  });
}
