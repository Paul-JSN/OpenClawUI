...
function stripYear(version: string): string {
...
export function renderApp(state: AppViewState) {
  ...
  const overviewAnalyticsView = buildUsageAnalyticsViewModel({
    ...
    usageCostSummary: state.overviewUsageCostSummary,
    usageStatus: state.overviewUsageStatus,
  ...
  return html`
    <div class="shell scanline ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      ...
      <aside class="nav ${state.settings.navCollapsed ? "nav--collapsed" : ""}">
        <div class="nav-brand">
          <div class="brand-logo" title="OpenClaw Gateway">
        ...
        ${TAB_GROUPS.map((group) => {
          const isSidebarCollapsed = state.settings.navCollapsed;
          const isGroupCollapsed = state.settings.navGroupsCollapsed[group.label] ?? false;
          const hasActiveTab = group.tabs.some((tab) => tab === state.tab);
          const visibleTabs = isSidebarCollapsed
            ? [...group.tabs]
            : isGroupCollapsed
              ? hasActiveTab
                ? group.tabs.filter((tab) => tab === state.tab)
                : []
              : [...group.tabs];
          const collapsedWithVisibleActive = isGroupCollapsed && visibleTabs.length > 0;
          return html`
            <div class="nav-group ${isGroupCollapsed ? "nav-group--collapsed" : ""} ${collapsedWithVisibleActive ? "nav-group--has-visible-item" : ""}">
              <button
                class="nav-label"
                @click=${() => {
        ...
        <div class="nav-footer">
          <span class="nav-footer__label">Version</span>
          <span class="nav-footer__value">${stripYear(openClawVersion)}</span>
        </div>
        <button
          class="nav-collapse-cta"
          @click=${() => {
            state.applySettings({
              ...state.settings,
              navCollapsed: !state.settings.navCollapsed,
            });
            window.dispatchEvent(new CustomEvent("openclaw-layout-transition"));
          }}
        ...
        >
          <span class="nav-collapse-cta__label">${state.settings.navCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}</span>
          <span class="nav-collapse-cta__icon">${state.settings.navCollapsed ? icons.sidebarExpand : icons.sidebarCollapse}</span>
        </button>
      </aside>
      <main class="content ${isAnalyticsTab ? "content--analytics" : "grid-bg"} ${isChat ? "content--chat" : ""}">
        ${
          availableUpdate
